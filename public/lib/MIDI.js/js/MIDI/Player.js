...
var scheduleTracking = function (channel, note, currentTime, offset, message, velocity, finger) {
	...
	for (var n = 0; n < length && messages < 100; n++) {
		...
		switch (event.subtype) {
			case 'noteOn':
				...
				//I add a 60 ms delay to every MIDI note start time so that the THREE.js animations sync up better.
				eventQueue.push({
					event: event,
					source: MIDI.noteOn(channel, event.noteNumber, event.velocity, (currentTime+60) / 1000 + ctx.currentTime), 
					interval: scheduleTracking(channel, note, queuedTime, offset, 144, event.velocity, event.finger)
				});
				messages ++;
				break;
			case 'noteOff':
				...
				eventQueue.push({
					...
					interval: scheduleTracking(channel, note, queuedTime, offset, 128, 0, event.finger)
				});
				break;
			default:
				break;
